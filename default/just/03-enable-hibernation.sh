...
configure_power_settings() {
  ...
  if grep -q -E "^HandleLidSwitch=suspend-then-hibernate$" "$LOGIND_CONF"; then
    ...
    sudo sed -i -E 's/^#HandleLidSwitch=.*/HandleLidSwitch=suspend-then-hibernate/' "$LOGIND_CONF"
  fi

  # --- Task 2: Configure HibernateDelaySec ---
  echo "Checking settings in ${SLEEP_CONF}..."
  if grep -q -E "^HibernateDelaySec=120$" "$SLEEP_CONF"; then
    echo "HibernateDelaySec is already set to 120 seconds."
  else
    echo "Setting HibernateDelaySec=120..."
    sudo sed -i -E 's/^#HibernateDelaySec=.*/HibernateDelaySec=120/' "$SLEEP_CONF"
  fi

  echo "Configuration complete. Restarting systemd-logind service to apply changes..."
